...
We begin by loading the cleaned dataset (the cleaning process is documented [here](data_clean.html)) and preparing it for visualization and analysis. Because the original dataset contains one row per person involved in a collision, we first filter it to retain only distinct `collision_id` values so that our analysis reflects the number of collision events rather than the number of individuals affected.
...
# Create the longer Dataset for Plotting
